...
<head>
    ...
    <style>
        ...
        /* Hide "Powered by Stoplight" branding (light DOM fallback) */
        a[href*="stoplight.io"],
        .sl-elements-api a[href*="stoplight.io"],
        [class*="powered"],
        [data-testid*="powered"] {
...
<script>
    // Strip "Powered by Stoplight" branding from shadow + light DOM.
    // Stoplight Elements renders the link as <a href="https://stoplight.io/...">Powered by Stoplight</a>
    // somewhere in the layout depending on version. We walk the whole tree (including shadow roots) and remove it.
    (function stripStoplightBranding() {
        const STOPLIGHT_HOST = 'stoplight.io';
        const TEXT_NEEDLES = ['Powered by Stoplight', 'powered by stoplight'];
        ...
        const walk = (root) => {
            ...
            all.forEach((el) => {
                ...
                if (el.tagName === 'A' && el.href && el.href.includes(STOPLIGHT_HOST)) {
                    el.remove();
                    return;
                }
                const txt = (el.textContent || '').trim();
                if (txt && txt.length < 80 && TEXT_NEEDLES.some((n) => txt.toLowerCase().includes(n.toLowerCase()))) {
        ...
        // Initial sweeps to catch late shadow DOM mounts
        const passes = [100, 300, 800, 2000, 5000];
        passes.forEach((delay) => setTimeout(() => walk(document.body), delay));
    })();
</script>
