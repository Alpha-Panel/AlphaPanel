    ...
    >
        <div class="flex max-h-[92vh] w-full max-w-lg flex-col rounded-xl border border-gray-700 bg-[#171717] text-white/90 shadow-2xl">
            <div class="flex items-center justify-between border-b border-white/10 px-5 py-4">
                <div>
                    <h5 class="text-base text-white">{{ t('Edit Alias') }}</h5>
                    <p v-if="alias" class="text-xs text-white/60">{{ alias.address }}</p>
                </div>
                <button
                    type="button"
...
watch(() => props.modelValue, (isOpen) => {
    if (isOpen && props.alias) {
        form.defaults({
            goto: props.alias.goto ?? '',
            is_active: props.alias.is_active ?? true,
        });
        form.reset();
        form.clearErrors();
    }
...
const submit = (): void => {
    ...
    form.put(route('domains.mail.aliases.update', [props.domainId, props.alias.id]), {
        preserveScroll: true,
        onSuccess: () => {
            emit('update:modelValue', false);
            emit('updated');
